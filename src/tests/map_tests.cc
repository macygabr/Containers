...
TEST(TestGroupName, at_1) {
  ...
  EXPECT_THROW(a.at(40), std::out_of_range);
  EXPECT_THROW(b.at(40), std::out_of_range);
  // a.printTree(a.root);
}
...
TEST(TestGroupName, Iterators1_char) {
  ...
  std::pair<int, int> a2 = {6, 6};
  a.insert(a1);
  std::map<int, int> b;
  std::pair<int, int> b1 = {1, 1};
  std::pair<int, int> b2 = {6, 6};
  b.insert(b1);
  a[1];
  a[2];
...
TEST(TestGroupName, erase_1) {
  ...
  for (it1 = a.begin(), it2 = b.begin(); s < 10 && it1 != a.end(); s++) {
    a.erase(it1++);
    b.erase(it2++);
  }

  for (it2 = b.begin(), it1 = a.begin(); it1 != a.end() || it2 != b.end();
       it1++, it2++) {
    // ASSERT_EQ(it1.node->node_key.first, it2->first);
  }
